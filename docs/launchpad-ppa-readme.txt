...
UPDATE (Oct 2020): The initial idea behind this PPA was to give Xenial users a way to keep their system quite up-to-date with newer software. In the time since this PPA was created (Aug 2019) many other PPAs have been added to this Launchpad site with "proper" backports of almost all the software that the "enhancement" process was to provide. Thus, this PPA is somewhat redundant, though the enhancement to "apt-cache policy" (show the version of a package in numerous different series with one terminal command) is still very useful.
...
This is a small package that adds the full set of standard repositories for eight other Ubuntu "series" to the installed system, as well as a small bash script called "apt-all" that allows usage of "sudo apt-get install" commands relative the repositories of any of the eight other series.

New "<series>-repositories-lo.pref" & "<series>-repositories-hi.pref.save" files (for the eight other series) are created in /etc/apt/preferences.d (simple binary switch pinning system) and new "official-repos-<series>.list" files are created in /etc/apt/sources.list.d/ (identical to default "official-package-repositories.list" files for each series, renamed to "official-package-repos-<series>.list" so that all can co-exist on the one system). As stated, one small script "apt-all" is also added (to /usr/bin).
...
Having nine series of packages and package information (specifically Xenial, Bionic, Cosmic, Disco, Eoan, Focal, Groovy, Hirsute, and Impish) available can be very useful in various situations (ie. see the "apt-cache" section below). From ubuntu-enhanced v0.9 the package is installable on any of nine series. Based on the currently installed series, the installation process of ubuntu-enhanced will determine the eight other series to add to the apt/dpkg system (with low pin priority being default) such that one can access packages (and package info) from all nine series.
...
So by using a simple renaming trick ("everything's a file" -- in this case, a set of files whose names show the toggle state for high pin preference of other series) in the event of power (or other) failure it is easy to see the current status. If any of the eight "<series>-repositories-hi.pref.save" files in /etc/apt/preferences.d do NOT have a ".save" extension (and instead end only with ".pref") then something failed.
...
meld:
  Installed: 3.20.3-1~16.04.sav0
  Candidate: 3.20.3-1~16.04.sav0
  Version table:
     3.20.3-1 50
         50 http://archive.ubuntu.com/ubuntu hirsute/universe amd64 Packages
         50 http://archive.ubuntu.com/ubuntu hirsute/universe i386 Packages
         50 http://archive.ubuntu.com/ubuntu impish/universe amd64 Packages
         50 http://archive.ubuntu.com/ubuntu impish/universe i386 Packages
 *** 3.20.3-1~16.04.sav0 500
     ...
     3.20.2-2 50
         50 http://archive.ubuntu.com/ubuntu groovy/universe amd64 Packages
         50 http://archive.ubuntu.com/ubuntu groovy/universe i386 Packages
     3.20.2-1ubuntu1 50
         50 http://archive.ubuntu.com/ubuntu focal/universe amd64 Packages
         50 http://archive.ubuntu.com/ubuntu focal/universe i386 Packages
     3.20.1-1 50
         50 http://old-releases.ubuntu.com/ubuntu eoan/universe amd64 Package
         50 http://old-releases.ubuntu.com/ubuntu eoan/universe i386 Packages
     3.20.0-2 50
...
This shows all meld versions from nine official series as well as from two Launchpad PPAs. The current installed package version is 3.20.3-1~16.04.sav0 which is from ppa:savoury1/gtk-xenial (GTK 3.22.30 and other upgrades, making Meld 3.20 buildable). There is also a version 3.18.3 for Xenial users who have not upgraded their GTK stack (Meld 3.18.3 is the highest possible with GTK < 3.20). All repos other than Xenial are currently pinned low (50) with Xenial having usual priority (500) as it is the installed series.

It's actually quite often very handy to have a fast terminal way to check the versions of any specified package(s) in the nine different Ubuntu series in question. As shown, this can be done very simply with "apt-cache policy" (on any of the nine series) after this ubuntu-enhanced package is installed.
...
Thus "apt-all" is simple to use and is all about giving a front-end for easy choice from all the packages of eight other series of Ubuntu than your own.
...
12/5/20 -- Added Groovy Gorilla to configuration and uploaded new 0.7 package allowing installation on any supported series from Xenial through to Groovy. This version also uses the correct server for old series past end of support.

31/10/20 -- Added Hirsute Hippo to configuration and uploaded new 0.8 package allowing installation on any supported series from Xenial through to Hirsute.

7/5/21 -- Added Impish Indri to configuration and uploaded new 0.9 package allowing installation on any supported series from Xenial through to Impish.
